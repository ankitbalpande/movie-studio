    ...
    useEffect(() => {
        if (data && data.id) {
            const fetchMovieDetails = async () => {
                try {
                    const response = await fetch(`https://api.themoviedb.org/3/tv/${data.id}?api_key=4ed3273ce23e349d820debb7786d3ac5`);
                    console.log(`https://api.themoviedb.org/3/tv/${data.id}?api_key=4ed3273ce23e349d820debb7786d3ac5`)
                        ...
                        <div className="flex sm:flex-row flex-col justify-center gap-5">
                            ...
                            <div className="sm:w-[calc(100%_-_384px)]  w-full p-5 text-white">
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex gap-3">
                                        <p className=''>Totel Episode : {movieDetails.last_episode_to_air.episode_number}</p>
                                    </div>
                                    <div className="pt-3">
                                        <p className='text-lg font-semibold pb-2'>Overview</p>
                                        <span>{movieDetails.overview || "N/A"}</span>
                                    </div>
                                    ...
                                    <div className="">
                                        <a href={movieDetails.homepage}> <span className='text-lg font-semibold'>Home page</span> : {movieDetails.homepage} </a>
                                    </div>
                                    <div className="mt-4">
...
export default ParticularTvShow